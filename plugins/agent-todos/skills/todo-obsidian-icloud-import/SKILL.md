...
invocation: user
argument-hint: "[vaultname] [todo-category]"
---
...
Collect the **vault name** and **todo category**. These may come from skill arguments or by asking the user.

- **Vault name** — The name of the Obsidian vault (directory name under iCloud Drive's Obsidian folder)
- **Todo category** — The subdirectory name under `agent-todos/` inside the vault (e.g., `misc`, `data`)

If both values are provided as arguments (e.g., `/todo-obsidian-icloud-import MyVault misc`), parse the first word as the vault name and the second as the todo category.
...
```bash
bash <base_directory>/scripts/import-obsidian-todos.sh --list "<vaultname>" "<todo-category>"
```

This prints one file path per line for each markdown file in the vault's `agent-todos/<todo-category>/` directory. Only files **not** prefixed with `DONE_` are listed (open todos).
...
2. **Use `/todo-creation`** to create a new local todo file in the same category:
   ```
   Skill: todo-creation
   Args: <todo-category> <title-from-file>
   ```
...
### 5. Delete Source Files (Optional)
...
```bash
bash <base_directory>/scripts/import-obsidian-todos.sh --delete "<vaultname>" "<todo-category>" "<filename>"
```
...
Importing from vault "WorkNotes" category "misc":

1. Script finds `~/Library/Mobile Documents/iCloud~md~obsidian/Documents/WorkNotes/agent-todos/misc/0001_fix_layout.md`
2. `/todo-creation misc "Fix layout"` creates `docs/agent-todos/misc/0005_fix_layout.md`
3. Content is copied from the Obsidian file to the local todo
4. Status is updated to `ready`
5. User confirms deletion; source file is moved to Trash
